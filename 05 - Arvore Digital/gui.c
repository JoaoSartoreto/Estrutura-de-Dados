...
int relatorio_anos_gui(Node *root, int anoInicial, int anoFinal, GtkListStore *model, GtkTreeIter *iter)
{
	...
	if(temp && temp->veiculo && temp->veiculo->ano >= anoInicial && temp->veiculo->ano <= anoFinal)
	{
		...
		gtk_list_store_set(model, iter,
			0, temp->veiculo->placa,
			1, temp->veiculo->renavam,
			2, temp->veiculo->marca,
			3, temp->veiculo->modelo,
			4, temp->veiculo->ano,
			5, temp->veiculo->cor,
			6, temp->veiculo->categoria,
			7, temp->veiculo->estado,
			8, temp->veiculo->cidade,
			-1);
			m1++;
	}
			
	for (i=0; i<N; i++) {
		m2 = m2 + relatorio_anos_gui(temp->children[i], anoInicial, anoFinal, model, iter); 
	}
	return m1 + m2;
...
void on_button_relatorio_anos_clicked(GtkWidget *widget, gpointer data)
{
	...
	qtd = relatorio_anos_gui((*info->rootAnt), anoInicio, anoFim, GTK_LIST_STORE(model), &iter);
...
int relatorio_estado_gui(Node *root, const char *estado, char finalP, GtkListStore *model, GtkTreeIter *iter)
{
	...
	for (i=0; i<N; i++) {
		m2 = m2 + relatorio_estado_gui(temp->children[i], estado, finalP, model, iter); 
	}
