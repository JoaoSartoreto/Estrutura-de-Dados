...
void on_button_buscar_procurar_clicked(GtkWidget *widget, gpointer data)
{
	...
	buscar = buscaPlaca(info->listaAnt, info->listaNov, placa);
...
void on_button_alterar_clicked(GtkWidget *widget, gpointer data)
{
	...
	const char *placa = gtk_entry_get_text(input);
	alterar = alterarPlaca(info->listaAnt, info->listaNov, placa);
	if(alterar)
	{	
		clock_gettime(CLOCK_MONOTONIC, &tend);
		sprintf(tempo, "Tempo: %ld nano segundos",(tend.tv_nsec) - (tstart.tv_nsec));
...
void on_button_relatorio_anos_clicked(GtkWidget *widget, gpointer data)
{
	...
	for(i=0;i<TAM;i++)
	{
		percorre = info->listaAnt[i].proximo;
		while(percorre)
		{	
			if(percorre->veiculo.ano >= anoInicio && percorre->veiculo.ano <= anoFim)
			{
				gtk_list_store_append(GTK_LIST_STORE(model), &iter);
				gtk_list_store_set(GTK_LIST_STORE(model), &iter,
					0, percorre->veiculo.placa,
					1, percorre->veiculo.renavam,
					2, percorre->veiculo.marca,
					3, percorre->veiculo.modelo,
					4, percorre->veiculo.ano,
					5, percorre->veiculo.cor,
					6, percorre->veiculo.categoria,
					7, percorre->veiculo.estado,
					8, percorre->veiculo.cidade,
					-1);
					qtd++;
	...
	for(i=0;i<TAM;i++)
	{
		percorre = info->listaNov[i].proximo;
		while(percorre)
		{	
			if(percorre->veiculo.ano >= anoInicio && percorre->veiculo.ano <= anoFim)
			{
...
void on_button_relatorio_estado_clicked(GtkWidget *widget, gpointer data)
{
	...
	while(finalP<='9')
	{
		for(j=0;j<TAM;j++)
		{
			percorre = info->listaAnt[j].proximo;
			while(percorre)
			{	
				if(strcmp(percorre->veiculo.estado,estado)==0 && percorre->veiculo.placa[6] == finalP)
				{
					gtk_list_store_append(GTK_LIST_STORE(model), &iter);
					gtk_list_store_set(GTK_LIST_STORE(model), &iter,
						0, percorre->veiculo.placa,
		...
		for(j=0;j<TAM;j++)
		{
			percorre = info->listaNov[j].proximo;
			while(percorre)
			{	
				if(strcmp(percorre->veiculo.estado,estado)==0 && percorre->veiculo.placa[6] == finalP)
				{
