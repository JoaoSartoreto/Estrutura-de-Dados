...
void on_button_buscar_procurar_clicked(GtkWidget *widget, gpointer data)
{
	...
	buscar = procurar_veiculo(info->tabelaAnt, info->tabelaNov, placa);
...
void on_button_alterar_clicked(GtkWidget *widget, gpointer data)
{
	...
	const char *placa = gtk_entry_get_text(input);
	alterar = alterarPlaca(info->tabelaAnt, info->tabelaNov, placa);
	if(alterar)
	{	
		clock_gettime(CLOCK_MONOTONIC, &tend);
		sprintf(tempo, "Tempo: %ld nano segundos",(tend.tv_nsec) - (tstart.tv_nsec));
...
void on_button_relatorio_anos_clicked(GtkWidget *widget, gpointer data)
{
	...
	for(i=0;i<TAM;i++)
		qtd = qtd + relatorio_anos_gui(info->tabelaAnt[i], anoInicio, anoFim, GTK_LIST_STORE(model), &iter);
	for(i=0;i<TAM;i++)
		qtd = qtd + relatorio_anos_gui(info->tabelaNov[i], anoInicio, anoFim, GTK_LIST_STORE(model), &iter);
...
void on_button_relatorio_estado_clicked(GtkWidget *widget, gpointer data)
{
	...
	{
		for(j=0;j<TAM;j++)
			qtd = qtd + relatorio_estado_gui(info->tabelaAnt[j], estado, finalP, GTK_LIST_STORE(model), &iter);
		for(j=0;j<TAM;j++)
			qtd = qtd + relatorio_estado_gui(info->tabelaNov[j], estado, finalP, GTK_LIST_STORE(model), &iter);
